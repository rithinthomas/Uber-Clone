  ...
  return (
    <SafeAreaView style={tw`bg-white flex-grow`}> 
      <View>
        ...
        <Text style={tw`text-center py-5 text-xl`}>Select a Ride - {travelTime?.distance?.text}</Text>
      </View>
          ...
          > 
            ...
            <Text style={tw`text-xl`}>
              {new Intl.NumberFormat('en-gb', {
                ...
                (travelTime?.duration.value * SURGE_CHARGE_RATE * multiplier) / 100
              )}
            </Text>
          </TouchableOpacity>
        )}
      ...
      <View style={tw`mt-auto border-t border-gray-200`}> 
        <TouchableOpacity disabled={!selected} style={tw`bg-black m-3 py-3 ${!selected && "bg-gray-300"}`}>
          <Text style={tw`text-white text-xl text-center`}>Choose {selected?.title}</Text>
        </TouchableOpacity>
      </View>
    </SafeAreaView>
  )
